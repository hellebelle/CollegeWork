...
{
	bloom *bloomFilter = (bloom *)malloc(sizeof(bloom));
...
{
	int value1 = hash_string(item, BLOOM_HASH1, this->size);
	int value2 = hash_string(item, BLOOM_HASH2, this->size);
	for(int i = 0; i< this->size; i++){
	   if((bitset_lookup(this->bit_vector, value1)==0) && (bitset_lookup(this->bit_vector, 		       value2)==0)){
		return 0;
	    }
	}
	return 1;
...
void bloom_add(struct bloom * this, char * item)
{
	int value1 = hash_string(item, BLOOM_HASH1, this->size);
	int value2 = hash_string(item, BLOOM_HASH2, this->size);
	bitset_add(this->bit_vector, value1);
	bitset_add(this->bit_vector, value2);
