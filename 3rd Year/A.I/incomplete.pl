...
astar(Node,Path,Cost) :- kb(KB), astar([[Node, [], 0 ]],Path,Cost,KB).

astar([[Node, Path, Cost]|_], [Node, Path], Cost, _) :- goal(Node).
astar([[Node, P, C]|Tail], Path, Cost, KB) :-   findall([X,[Node|P],Sum], (arc(Node, X, Y, KB), Sum is Y+C), Children),
                                                addtofrontier(Children, Tail, Temp),
                                                minSort(Temp, [[N1, P1, C1]|T1]),
                                                astar([[N1,P1,C1]|T1], Path, Cost, KB).
